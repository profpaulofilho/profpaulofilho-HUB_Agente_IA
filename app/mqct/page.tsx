...
export default async function MQCTPage() {
  ...
  const groupedWithClaude = grouped.map((cat: any) => {
    ...
    const claudeCard = {
      ...
      name: 'MQCT - CLAUDE',
      description:
        'Acesse os agentes MQCT no ambiente Claude IA. Este card abre o hub externo em outra aba.',
      provider: 'Anthropic',
      platform: 'Claude',
          ...
          <div key={cat.id} style={{ marginBottom: 36, animation: `fadeUp 0.6s ease ${0.1 + ci * 0.08}s both` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.2)', whiteSpace: 'nowrap' }}>
                {cat.agents.length} agente{cat.agents.length !== 1 ? 's' : ''}
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(360px,1fr))', gap: 14 }}>
              {cat.agents.map((agent: any) => {
                ...
                const badgeBg = claude
                  ? 'rgba(245,158,11,0.12)'
                  : gpt
                    ? 'rgba(37,99,235,0.12)'
                    : 'rgba(124,58,237,0.12)'

                const badgeBorder = claude
                  ? 'rgba(245,158,11,0.25)'
                  : gpt
                    ? 'rgba(37,99,235,0.25)'
                    : 'rgba(124,58,237,0.25)'

                const badgeColor = claude
                  ? '#fcd34d'
                  : gpt
                    ? '#93c5fd'
                    : '#c4b5fd'
