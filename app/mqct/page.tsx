...
const EXCLUDED = ['claude', 'assis', 'organização de ucs', 'organizacao de ucs', 'plano de aula']
...
function isClaude(provider: string, platform: string, name?: string) {
  const p = (platform || '').toLowerCase()
  const pr = (provider || '').toLowerCase()
  const n = (name || '').toLowerCase()
  return p.includes('claude') || pr.includes('anthropic') || n.includes('claude')
}
...
export default async function MQCTPage() {
  ...
  const groupedWithClaude = grouped.map((cat: any) => {
    if ((cat.name || '').toLowerCase() !== 'cursos técnicos') {
      return cat
    }
    ...
    return {
      ...
      agents: [
        ...cat.agents,
        {
          id: 'mqct-claude-custom-card',
          name: 'MQCT - CLAUDE',
          description: 'Acesse os agentes MQCT no ambiente Claude IA em outra aba.',
          provider: 'Anthropic',
          platform: 'Claude',
          external_url: 'https://profpaulofilho.github.io/hub-artifacts_IA/',
          isCustomClaude: true,
        },
      ],
    }
  ...
  return (
    <main style={{ minHeight:'100vh', background:'#05080f', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{position:'relative',zIndex:10,maxWidth:1200,margin:'0 auto',padding:'28px 24px'}}>
        ...
        {/* CATEGORIES + AGENTS */}
        {groupedWithClaude.map((cat: any, ci: number) => (
          <div key={cat.id} style={{marginBottom:36,animation:`fadeUp 0.6s ease ${0.1 + ci*0.08}s both`}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(360px,1fr))',gap:14}}>
              {cat.agents.map((agent: any) => {
                ...
                const claude = isClaude(agent.provider, agent.platform, agent.name)
                const btnBg = claude
                  ? 'linear-gradient(135deg,#d97706,#f59e0b)'
                  : gpt
                    ? 'linear-gradient(135deg,#1d4ed8,#3b82f6)'
                    : 'linear-gradient(135deg,#7c3aed,#8b5cf6)'
                const btnShadow = claude
                ...
                return (
                  <div key={agent.id} className="agent-card" style={{
                    ...
                    <div style={{position:'relative',flexShrink:0}} className="avatar-wrap">
                      <div style={{width:52,height:52,borderRadius:14,background:claude?'rgba(245,158,11,0.16)':gpt?'rgba(37,99,235,0.18)':'rgba(124,58,237,0.18)',border:`1px solid ${claude?'rgba(245,158,11,0.3)':gpt?'rgba(37,99,235,0.3)':'rgba(124,58,237,0.3)'}`,display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',boxShadow:`0 4px 14px ${claude?'rgba(245,158,11,0.2)':gpt?'rgba(37,99,235,0.2)':'rgba(124,58,237,0.2)'}`}}>
                        {claude
                          ? <img src="/CLAUDE.png" alt="Claude" style={{width:'100%',height:'100%',objectFit:'cover',borderRadius:14}}/>
                          : gpt
                      ...
                      {/* Tooltip com cursos */}
                      {!agent.isCustomClaude && extractCourses(agent.name).length > 0 && (
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
                        <span style={{fontSize:10,fontWeight:700,background:claude?'rgba(245,158,11,0.12)':gpt?'rgba(37,99,235,0.12)':'rgba(124,58,237,0.12)',border:`1px solid ${claude?'rgba(245,158,11,0.25)':gpt?'rgba(37,99,235,0.25)':'rgba(124,58,237,0.25)'}`,color:claude?'#fcd34d':gpt?'#93c5fd':'#c4b5fd',borderRadius:20,padding:'2px 9px'}}>{agent.provider}</span>
