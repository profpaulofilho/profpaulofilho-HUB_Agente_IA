    ...
    <main style={S.page}>
      <style>{`
        ...
        * { font-family: 'Sora', sans-serif; }
        ...
        .cat-card:hover { background: rgba(37,99,235,0.15) !important; border-color: rgba(37,99,235,0.4) !important; }
      ...
      <div style={{ maxWidth:1360, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, gap:12, flexWrap:'wrap' }}>
          ...
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            {[
              { href:'/admin/agentes', label:'Gerenciar agentes' },
              { href:'/usuarios', label:'Usuários' },
            ].map(b => (
              <Link key={b.href} href={b.href} className="nav-btn" style={{
        ...
        <div className="anim" style={{
          ...S.card, ...S.cardPad, marginBottom:16,
          background:'linear-gradient(135deg, rgba(29,78,216,0.25) 0%, rgba(124,58,237,0.15) 100%)',
          border:'1px solid rgba(37,99,235,0.25)',
          display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:16,
        }}>
          <div>
            <p style={{ margin:'0 0 6px', fontSize:11, fontWeight:700, color:'#60a5fa', textTransform:'uppercase', letterSpacing:'0.8px' }}>
              Área administrativa · SENAI Bahia
            </p>
            <h1 style={{ margin:'0 0 6px', fontSize:28, fontWeight:800, color:'#fff', letterSpacing:'-0.8px' }}>
              Hub de Agentes de IA
            </h1>
            <p style={{ margin:0, fontSize:13, color:'rgba(255,255,255,0.45)', fontWeight:300 }}>
              Logado como <span style={{ color:'#93c5fd' }}>{user.email}</span>
            </p>
          </div>
          <div style={{ display:'flex', gap:8 }}>
            <div style={{ width:8, height:8, borderRadius:'50%', background:'#22c55e', boxShadow:'0 0 8px #22c55e', marginTop:6 }} />
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:12, marginBottom:16 }}>
          {[
            ...
            <div key={m.label} style={{ ...S.card, ...S.cardPad, textAlign:'center' }}>
              ...
              <div style={{ fontSize:11, color:'rgba(255,255,255,0.35)', marginTop:4, fontWeight:400 }}>{m.label}</div>
            </div>
          ))}
        </div>
                ...
                {topAgents.map((item, i) => (
                  <div key={item.name}>
                    ...
                    <div style={{ width:'100%', height:8, borderRadius:4, background:'rgba(255,255,255,0.06)' }}>
                      <div style={{ width:`${(item.total/maxBar)*100}%`, height:'100%', borderRadius:4, background:'linear-gradient(90deg,#1d4ed8,#60a5fa)', transition:'width 0.5s ease' }} />
                    </div>
                  </div>
                ))}
              </div>
            ) : <div style={{ color:'rgba(255,255,255,0.25)', fontSize:13, padding:'20px 0' }}>Nenhum acesso registrado ainda.</div>}
          </div>
        ...
        {/* CATEGORIES GRID */}
        <div style={{ ...S.card, ...S.cardPad, marginBottom:16 }}>
          <h2 style={{ ...S.h2, marginBottom:16 }}>Categorias de agentes</h2>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:12 }}>
            {groupedAgents.map((cat: any) => (
              <a key={cat.id} href={`#cat-${cat.slug}`} className="cat-card" style={{
                ...
                <div style={{ fontSize:11, color:'#60a5fa', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:6 }}>Categoria</div>
                <div style={{ fontSize:18, fontWeight:700, color:'#fff', marginBottom:6 }}>{cat.name}</div>
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.4)', lineHeight:1.5, marginBottom:12 }}>{cat.description || 'Agentes desta categoria'}</div>
                <div style={{ display:'inline-flex', alignItems:'center', gap:6, background:'rgba(37,99,235,0.2)', borderRadius:20, padding:'4px 10px', fontSize:11, color:'#93c5fd', fontWeight:600 }}>
                  {cat.agents.length} agente(s)
                </div>
              </a>
            ))}
          </div>
        ...
        {groupedAgents.map((cat: any) => (
          <div key={cat.id} id={`cat-${cat.slug}`} style={{ ...S.card, ...S.cardPad, marginBottom:16 }}>
            <div style={{ marginBottom:18, paddingBottom:16, borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
              ...
              <h2 style={{ margin:'0 0 4px', fontSize:22, fontWeight:800, color:'#fff', letterSpacing:'-0.3px' }}>{cat.name}</h2>
              <p style={{ margin:0, fontSize:12, color:'rgba(255,255,255,0.35)' }}>{cat.description || 'Agentes disponíveis.'}</p>
            </div>

            <div style={{ display:'grid', gap:10 }}>
              {cat.agents.length > 0 ? cat.agents.map((agent: any) => (
                <div key={agent.id} className="agent-row" style={{
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <h3 style={{ margin:'0 0 5px', fontSize:15, fontWeight:700, color:'#f1f5f9' }}>{agent.name}</h3>
                    {agent.description && (
                      <p style={{ margin:'0 0 6px', fontSize:12, color:'rgba(255,255,255,0.35)', lineHeight:1.5 }}>{agent.description}</p>
                    )}
                    <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                      <span style={{ fontSize:10, fontWeight:600, background:'rgba(37,99,235,0.15)', border:'1px solid rgba(37,99,235,0.2)', color:'#93c5fd', borderRadius:20, padding:'3px 8px' }}>{agent.provider}</span>
                      <span style={{ fontSize:10, fontWeight:600, background:'rgba(124,58,237,0.15)', border:'1px solid rgba(124,58,237,0.2)', color:'#c4b5fd', borderRadius:20, padding:'3px 8px' }}>{agent.platform}</span>
                    </div>
                  </div>
                  <Link href={`/agentes/${agent.id}`} className="open-btn" style={{
                ...
                <div style={{ padding:'20px 0', color:'rgba(255,255,255,0.25)', fontSize:13 }}>Nenhum agente nesta categoria.</div>
              )}
            </div>
          </div>
        ))}
